// Changes.100SaaS (Tool 12) — PocketBase hooks
// Subdomain: changes.100saas.com
//
// Implements:
// - Shared Kernel: RevenueCat entitlement sync webhook
// - Tool 12: Basic validation for entries writes (server-side safety)
...
function normalizeTags(tags) {
  if (!tags) return [];
  if (Array.isArray(tags)) return tags.slice(0, 10).map((t) => String(t).slice(0, 32));
  return [];
}
...
routerAdd("GET", "/api/billing/status", (c) => {
  const k = require(__hooks + "/_shared/kernel.js");
  return k.handleBillingStatus(c, { toolSlug: "changes" });
});
routerAdd("POST", "/api/billing/checkout", (c) => {
  const k = require(__hooks + "/_shared/kernel.js");
  return k.handleBillingCheckout(c, { toolSlug: "changes" });
});
...
routerAdd("POST", "/api/webhooks/stripe/billing", (c) => {
  const k = require(__hooks + "/_shared/kernel.js");
  return k.handleStripeBillingWebhook(c, { toolSlug: "changes" });
});
routerAdd("GET", "/api/stripe/connect/start", (c) => {
  const k = require(__hooks + "/_shared/kernel.js");
  return k.handleStripeConnectStart(c, { toolSlug: "changes" });
});
routerAdd("GET", "/api/stripe/connect/callback", (c) => {
  const k = require(__hooks + "/_shared/kernel.js");
  return k.handleStripeConnectCallback(c, { toolSlug: "changes" });
});
routerAdd("GET", "/api/tool/health", (c) => {
  const k = require(__hooks + "/_shared/kernel.js");
  const required = ["tenants", "memberships", "stripe_connections", "audit_logs", "entries"];
  const missing = k.listMissingCollections(required);
  if (missing.length) {
    return c.json(500, { ok: false, tool: "changes", toolNumber: 12, missingCollections: missing });
  }
  return c.json(200, { ok: true, tool: "changes", toolNumber: 12 });
});

// Tool 12: demo seed (owner-only)
routerAdd("POST", "/api/changes/demo/seed", (c) => {
  const k = require(__hooks + "/_shared/kernel.js");
  const user = k.getAuthRecord(c);
  if (!user) return c.json(401, { error: "unauthorized", message: "not_authenticated" });

  const now = Date.now();
  if (!k.allowRequest(`changes_demo_seed:user:${user.id}`, now, 10, 10 * 60 * 1000)) return c.json(403, { error: "forbidden", message: "rate_limited" });
  ...
  const col = $app.dao().findCollectionByNameOrId("entries");
  const items = [
    { title: "Launched public beta", content_html: "<p>Batch 1 is live for feedback.</p>", tags: ["launch"] },
    { title: "Added team invites", content_html: "<p>Invite teammates via link tokens.</p>", tags: ["teams"] },
  ];
  for (const it of items) {
    const rec = new Record(col);
    rec.set("tenant", tenantId);
    rec.set("title", it.title);
    rec.set("content_html", it.content_html);
...
onRecordBeforeCreateRequest("entries", (e) => {
...
onRecordBeforeUpdateRequest("entries", (e) => {
  const { json, safeString, badRequest, unauthorized, forbidden, internalError, getEnv, clampText, normalizeSlug, normalizeEmail, normalizeTags, getClientIp, bestEffortClientIp, randToken, randCode, validateUploadToken, clamp01to100, clampNum, requireRole, parseJsonBody, clampEnum, clampIntervalDays, normalizeKeyName, isOverdue, isExpired, setTenantSlug, setTenantSlugFromTenantId, allowRequest, toDateOnlyIso, msToIso, findTenantForStripeEvent } = require(__hooks + "/_shared/kernel.js");
  const user = e.httpContext?.auth?.record || null;
  const tenantId = e.record.getString("tenant");
  if (user && tenantId) requireRole(user, tenantId, "admin");
